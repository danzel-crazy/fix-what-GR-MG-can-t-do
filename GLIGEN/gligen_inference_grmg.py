import sys
sys.path.append('/tmp2/young91319/GR-MG/GLIGEN/ldm')
...
def load_ckpt(ckpt_path):
    ...
    # Ignore unexpected key "transformer.text_model.embeddings.position_ids"
    text_encoder_state_dict = saved_ckpt["text_encoder"]
    if "transformer.text_model.embeddings.position_ids" in text_encoder_state_dict:
        del text_encoder_state_dict["transformer.text_model.embeddings.position_ids"]
...
def get_clip_feature(model, processor, input, is_image=False):
    ...
    if is_image:
        ...
        if which_layer_image == 'after_reproject':
            feature = project( feature, torch.load('/tmp2/young91319/GR-MG/GLIGEN/projection_matrix').cuda().T ).squeeze(0)
...
@torch.no_grad()
def prepare_batch(meta, batch=1, max_objs=100):
...
# results = [{'scores': tensor([0.6015], device='cuda:0'), 'labels': ['a white object'], 'boxes': tensor([[-6.1264e-02,  6.5979e+01,  3.6347e+02,  2.9988e+02]], device='cuda:0')}]

def inpainting_image(resized_image, location, arm_image, target_image, lang_annotation):
    meta = dict(
        ckpt = "/tmp2/young91319/GR-MG/GLIGEN/diffusion_pytorch_model_inpainting.bin",
        input_image = resized_image,
        prompt = lang_annotation,
        # phrases =   ['corgi', 'cake'],
        images = [ arm_image, target_image ],
        locations = location,
        save_folder_name="inpainting_box_text"
    )
    starting_noise = torch.randn(args.batch_size, 4, 64, 64).to(device)
    starting_noise = None
    goal_image = run(meta, args, starting_noise)
    return goal_image

def  inpainting_image_stage2(resized_image, location, arm_image, lang_annotation):
    meta = dict(
        ckpt = "/tmp2/young91319/GR-MG/GLIGEN/diffusion_pytorch_model_inpainting.bin",
        input_image = resized_image,
        prompt = lang_annotation,
...
if __name__ == "__main__":
    ...
    meta_list = [ 
        ...
        # dict(
        #     ckpt = "./diffusion_pytorch_model_inpainting.bin",
